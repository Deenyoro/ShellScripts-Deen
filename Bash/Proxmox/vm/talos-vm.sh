...
function error_handler() {
  ...
  local line_number="$1"
  local command="$2"
  echo -e "\n${RD}[ERROR]${CL} at line $line_number: exit code $exit_code, while executing: $command\n"
  cleanup_vmid
  exit $exit_code
}

function cleanup_vmid() {
  # If a VMID was set and exists, stop and destroy it to avoid clutter.
...
function header_continue() {
  header_info
  read -r
}

function check_vmid {
  # Increment NEXTID if it's already in use.
...
function pve_check() {
  # Check Proxmox version (requires at least 8.1)
...
function advanced_settings() {
  ...
  DISK_CACHE=$(whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
    ...
    "none" "None (recommended for data integrity)" ON \
    "writeback" "Better performance, higher risk" OFF \
    3>&1 1>&2 2<&3) || exit_script

  CPU_TYPE=$(whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
    --title "CPU MODEL" --radiolist "CPU model:" 10 60 2 \
    "host" "Host (use host CPU features)" ON \
    "kvm64" "Generic, less optimized" OFF \
    3>&1 1>&2 2<&3) || exit_script
  ...
  MAC=$(whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
    --inputbox "MAC Address (Auto-generated: $GEN_MAC)" 8 60 "$GEN_MAC" \
  ...
  VM_TAG=$(whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
    --inputbox "VM Tag for identification (Default: kubernetes)" 8 60 "kubernetes" \
...
function start_script() {
  if whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
      --title "SETTINGS" --yesno "Use Default Settings?" --defaultno 10 60; then
    default_settings
  else
    advanced_settings
  fi
...
if ! whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
  --title "Talos VM" \
  --yesno "This will create a new Talos VM. Proceed?" 10 58; then
...
RELEASE_DATE=$(curl -s https://api.github.com/repos/siderolabs/talos/releases/latest | \
  grep '"published_at"' | sed -E 's/.*"([^"]+)".*/\1/' | cut -d'T' -f1 | sed 's/-//g')
...
if whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
  --title "ISO SELECTION" \
  --yesno "Would you like to download the Talos ISO from the internet?\n\nChoose 'No' to select a locally available ISO." 10 60; then
  # User chose to download
  if [[ -f "$ISO_PATH" ]]; then
    msg_ok "ISO file already exists: $ISO_FILE"
  else
    msg_info "Downloading Talos ISO"
    if ! wget -q --show-progress "$ISO_URL" -O "$ISO_PATH"; then
      msg_error "Failed to download Talos ISO."
      # Prompt user to select local ISO if download fails
      if whiptail --backtitle "Proxmox VE Talos Linux Install Script" --title "LOCAL ISO" \
        ...
        ISO_FILE=$(whiptail --backtitle "Proxmox VE Talos Linux Install Script" \
